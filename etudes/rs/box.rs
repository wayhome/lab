...
fn main() {
    ...
    println!("Unboxed point occupies {} bytes in the stack", size_of_val(&boxed_point));

    //unboxing via deconstruction
    let box another_unboxed_point = boxed_point;
    println!("Another unboxed point occupies {} bytes in the stack", size_of_val(&another_unboxed_point));
